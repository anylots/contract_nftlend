...
存入资产，并在AssetsRecordHub记录用户的已存入资产。
...
| nftTokenId         | uint256 | nftTokenId                                             |
| onBehalfOf     | address | 转账的目标地址，即债权人，默认填写 `msg.sender`     |
...
| onBehalfOf       | address | 债务记录地址，即债务偿还人，默认填写 `msg.sender` |
...
function _executeBorrow(ExecuteBorrowParams memory vars) internal {
  ...
  // 代表是否是该用户在此资产上的第一笔借贷
  bool isFirstBorrowing = AssetsRecordHub.lendAsset(reserve.assetName, vars.amount, vars.user);
...
```solidity
/**
  * @notice Repays a borrowed `amount` on a specific reserve, burning the equivalent debt record owned
  * - E.g. User repays 100 USDC, burning 100 variable/stable debt tokens of the `onBehalfOf` address
  * @param asset The address of the borrowed underlying asset previously borrowed
  * @param amount The amount to repay
  * - Send the value type(uint256).max in order to repay the whole record for `asset` on the specific `debtMode`
...
function repay(
  ...
  // 调用AssetsRecordHub.repayAsset()核销债务
  bool isFirstBorrowing = AssetsRecordHub.repayAsset(reserve.assetAddress, vars.amount, vars.user);

  reserve.updateInterestRates(reserve.asset, assetAddress, paybackAmount, 0);

  if (stableDebt.add(variableDebt).sub(paybackAmount) == 0) {
    _usersConfig[onBehalfOf].setBorrowing(reserve.id, false);
  }

  IERC20(asset).safeTransferFrom(msg.sender, assetAddress, paybackAmount);
...
function swapBorrowRateMode(address asset, uint256 rateMode) external override whenNotPaused {
  ...
  // 获取偿还人的债务数量，分别调用 stableDectToken 和 variableDebtRecord 的 balanceOf 方法
  ...
  reserve.updateInterestRates(asset, reserve.assetAddress, 0, 0);
...
contract Liquidator {
        ...
        external
    {
        ...
        lendingPool.liquidationCall(_collateral, _reserve, _user, _purchaseAmount, assetAddress);
    }
}
```
...
function flashLoan(
  ...
  address[] memory assetAddress = new address[](assets.length);  // 缓存 asset 地址
  ...
  for (vars.i = 0; vars.i < assets.length; vars.i++) {  // 遍历缓存asset地址 和 累加手续费，并先将贷款转给调用者
    assetAddresss[vars.i] = _reserves[assets[vars.i]].assetAddress;

    premiums[vars.i] = amounts[vars.i].mul(_flashLoanPremiumTotal).div(10000);  // premium = amount * 9 / 10000

    AssetsRecordHub(assetAddresss[vars.i]).transferUnderlyingTo(receiverAddress, amounts[vars.i]); // 先将贷款转给调用者
  }
  ...
  for (vars.i = 0; vars.i < assets.length; vars.i++) {  // 遍历入参，执行闪电贷逻辑
    ...
    vars.assetAddress = assetAddress[vars.i];
    ...
    if (DataTypes.InterestRateMode(modes[vars.i]) == DataTypes.InterestRateMode.NONE) { // mode = 0, 代表执行闪电贷
      ...
      _reserves[vars.currentAsset].cumulateToLiquidityIndex(
        IERC20(vars.assetAddress).totalSupply(),
        vars.currentPremium
      );
...
function rebalanceStableBorrowRate(address asset, address user) external override whenNotPaused {
  ...
  address assetAddress = reserve.assetAddress;
  ...
  reserve.updateInterestRates(asset, assetAddress, 0, 0);
